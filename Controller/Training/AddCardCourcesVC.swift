...
class AddCardCourcesVC: UIViewController{
        ...
        AddCardManager.shared.AddCard(withParametrs: param) { (isAdded, error) in
            if isAdded{
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5, execute: {
                    let param:[String:String] = ["course_id":(self.TrainingSportDetailData?.courseId)!,"user_id":(UserManager.shared.currentUser?.user_id)!,"amount":"\(self.Total)","transaction_id":"12345","order_status":"completed","book_avail_ids":self.book_avail_timeIds.joined(separator: ","),"include_place":self.isIncludPlace,"include_sport_tools":self.isIncludSport_tool]
                    TrainingManager.shared.BookCources(param: param, completion: { (isBook, error) in
                        if isBook == true{
                            ...
                            alert.addAction(UIAlertAction(title: "Ok", style: UIAlertActionStyle.default, handler: {(action:UIAlertAction!) in
                                ...
                                ivc.Court_ID = (self.TrainingSportDetailData?.courseId)!
                                ivc.ReviewType = "cources"
